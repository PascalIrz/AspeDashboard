...
pak::pkg_install(c(
    "PascalIrz/aspe", "karthik/rdrop2", "OFB-IdF/templatesOFB",
    "OFB-IdF/AspeDashboard",
    "MaelTheuliere/COGiter",
    "httr", "sf", "rmapshaper", "dplyr", "purrr", "usethis", "stringr", "progress", "lubridate", "tidyr", "hubeau",
  ...
  cat("\n#", endpoint, file = "log")
    ...
    if (inherits(results, "try-error")) {
        ...
        results <- purrr::map(
            stations,
            .f = function(sta) {
              cat("\nstation ", sta, file = "log", append = TRUE)
              ...
              if (endpoint == "operations") {
                ...
                cat("  OK", file = "log", append = TRUE)
              } else {
                results_sta <- purrr::map(
                  unique(operations$code_operation),
                  .f = function(op) {
                    cat("\n    operation ", op, file = "log", append = TRUE)
                    n_try = 1
...
**Idealement: Un gros téléchargement avec l'ensemble des données en sauvant la date de l'export, puis régulièrement récupère que les données dont la date de modification des opérations est postérieure à la date d'export**

```{r}
if (file.exists("data_hubeau.rda")) {
    load("data_hubeau.rda")
}
  

if (download_hubeau) {
  dept_test <- NULL # change for a small department (e.g. 93) for code test

  if (exists("date_export")) {
    new_operations <- get_data_poissons(
      endpoint = "operations",
      date_modification_operation_min = date_export, code_departement = dept_test,
  ...
  if (exists("operations")) {
    ...
    indicateurs <- indicateurs |> 
      dplyr::filter(!code_operation %in% new_operation$code_operation) |> 
  ...
  save(operations, stations, indicateurs, observations, date_export, file = "data_hubeau.rda")
  # vroom::vroom_write(observations, "observations.csv", delim = ";")
  }
...
```{r}
rm(stations, operations, observations)
```
...
save(
  date_export, pop_geo, captures, ipr , metriques, codes_especes, carte_operations, LegendeEspeces, LegendeIpr, LegendeDistribution, 
  file = "../inst/app/data/dataApp.rda")
```
